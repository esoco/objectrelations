...
public class Functions {
	...
	public static <L, R> BinaryFunction<L, R, L> doAll(
		...
		List<BinaryFunction<? super L, ? super R, ?>> aFunctions = new ArrayList<BinaryFunction<? super L, ? super R, ?>>();
	...
	public static <I, O> BinaryFunction<I, String, O> error(
		...
		return new AbstractBinaryFunction<I, String, O>(sMessage, "error") {
			final Constructor<? extends Exception> rConstructor = ReflectUtil.getPublicConstructor(rExceptionClass,
					String.class);
	...
	public static Function<?, ?> firstInChain(Function<?, ?> rFunction) {
		if (rFunction instanceof FunctionChain<?, ?, ?>) {
			rFunction = firstInChain(((FunctionChain<?, ?, ?>) rFunction).getInner());
		}
	...
	public static Function<?, ?> lastInChain(Function<?, ?> rFunction) {
		if (rFunction instanceof FunctionChain<?, ?, ?>) {
			rFunction = lastInChain(((FunctionChain<?, ?, ?>) rFunction).getOuter());
		}
