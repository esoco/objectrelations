...
public class CalendarFunctions {
	 ...
	 * @param bMaximum       TRUE to reset fields to their maximum value,
	 *                       FALSE
	 *                       for the minimum
	 * @return The calendar for concatenation
	 */
	public static Calendar resetBelow(int nCalendarField, Calendar rCalendar,
		boolean bMaximum) {
		int nFieldIndex = 0;
		...
		if (nFieldIndex < ORDERED_CALENDAR_FIELDS.length) {
			...
			for (int i = nFieldIndex - 1; i >= 0; i--) {
				...
				rCalendar.set(nField, bMaximum ?
				                      rCalendar.getActualMaximum(nField) :
